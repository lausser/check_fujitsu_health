...
$plugin->add_arg(
   ...
   the time between two runs of check_fujitsu_health is the base for calculations.
...
if (! $plugin->opts->statefilesdir) {
  if (exists $ENV{OMD_ROOT}) {
    $plugin->override_opt('statefilesdir', $ENV{OMD_ROOT}."/var/tmp/check_fujitsu_health");
  } else {
